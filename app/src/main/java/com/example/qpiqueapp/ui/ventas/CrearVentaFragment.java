...
public class CrearVentaFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Adapter solo lectura
        adapter = new CarritoAdapter(new ArrayList<>(), null);
        binding.rvProductosVenta.setLayoutManager(new LinearLayoutManager(requireContext()));
        ...
        ventasViewModel.getClienteSeleccionado().observe(getViewLifecycleOwner(), cliente -> {
            if (cliente != null) {
                binding.actCliente.setText(cliente.getNombre() + " " + cliente.getApellido());
                binding.actCliente.setEnabled(false);
                crearVentaViewModel.setCliente(cliente);
            } else {
                binding.actCliente.setText("Sin cliente seleccionado");
                binding.actCliente.setEnabled(true);
            }
        });

        // Botón confirmar venta
        binding.btnCrearVenta.setOnClickListener(v -> {
            List<Productos> carrito = carritoViewModel.getCarrito().getValue();
            ...
            crearVentaViewModel.crearVenta(carrito);
        });
        ...
        crearVentaViewModel.getError().observe(getViewLifecycleOwner(), error -> {
            if (error != null) {
